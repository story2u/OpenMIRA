...
func MarkdownReport(report ComparisonReport) string {
	...
	builder.WriteString("This report compares Python and Go replay samples at event-pair granularity.\n\n")
...
func writeReplayResultTable(builder *strings.Builder, results []ComparisonResult) {
	...
	builder.WriteString("| Index | Match | Python | Go | Diffs |\n")
	builder.WriteString("| --- | --- | --- | --- | --- |\n")
	if len(results) == 0 {
	...
	for _, result := range results {
		builder.WriteString(fmt.Sprintf(
			...
			result.Index,
			result.Match,
			escapeTable(formatEventSummary(result.Python)),
