...
RN 启动时并行恢复 SecureStore 会话和迁移本地 `radar.db`。当前 SQLite v7 建立按 `owner_id` 隔离的原始
...
恢复状态；另以应用自定义、版本化 entry 保存 30 天交互式 Agent 本地会话，并以 append-only 信息胃口
事件折叠 preference/intent/example/session/decision/shadow/temporary focus 与一次性教学 UI 状态。分析 run token 可使用
...
`client.reactNative=true`/`sqlite.schema=7`，并如实报告 `agent.submitAnalysis=true`、
`agent.streaming=true`、`agent.runtime=pi-0.80.6`、`agent.schema=1`、`agent.interactive=true` 与
`agent.interactiveSchema=4` 与 `signalAppetite.eventSchema=1`（表示客户端最高支持版本，服务端仍可选择更低版本）；
...
| `SignalAppetiteEvent` | 信息胃口多设备 content-free 事件流 | owner + event ID 唯一；绑定 active device、独立单调 cursor、64 KiB payload 与 event/schema/aggregate version；内容一致才幂等，冲突拒绝；默认 rollout 关闭 |
...
- RN SQLite v7 保存恢复所需的最小 run/input 元数据，短期 bearer 只进 device-only SecureStore；claim 后
...
- `/api/v1/agent/interactive/gateway/v1/chat/completions` 只接受 purpose turn token、固定 alias/system
  ...
  只再增加 `send_reply(opportunity_id,text)`；v4 增加 15 个版本化信息胃口只读/内部工具。工具参数不接受 token、owner、幂等键、channel 或
...
- v4 的教学样本只追加本地事件，propose/simulate 不激活稳定偏好；`apply_appetite_change` 使用独立
  `beforeToolCall` 一次性确认并绑定 tool call + candidate version，不能与外发 purpose token 互换。L0/L1
  evaluator 离线可运行，边界消息在云端不可用时进入 inbox/digest 而不是 suppress。
  `GET/POST /sync/signal-appetite/events` 以独立 cursor 同步 content-free 事件，客户端先 push pending 再 pull/fold；
  `SIGNAL_APPETITE_SYNC_ENABLED` 与 capability 默认关闭，关闭时单设备教学和过滤继续工作。
- `agentToolsAvailable` 默认 false，只有 beta/gateway、正数独立额度、active allowlisted device、SQLite v7、
  ...
  `3:interactive-approved-send-v3` 或 `4:interactive-signal-appetite-v4` 的成对配置。v3+ 仍强制外发工具受两个外发开关；关闭任一门禁会阻止批准和
