...
| 商机详情 | Web/RN `/opportunity/[id]` | `GET /opportunities/{id}` | 已实现 | Web 与 RN 均通过共享严格 decoder 独立读取，不依赖列表预热；RN 覆盖 UUID 深链、登录返回、404/归档、Agent 发现、loading/error/retry 与刷新，并在 iOS Release 模拟器用确定性 fixture 验证。P3 S4 已实现 capability 门控的跨启动 SQLite 详情回退，生产灰度与真机证据仍待完成 |
...
| 用户级通知偏好 | Web/iOS/Android/RN 设置 | `GET /settings/me`、`PATCH /settings/notifications` | 部分实现 | `user_notification_preferences` 表按 owner 持久化，Web/RN 共用严格契约并在失败时恢复旧值。RN 已提供用户显式授权的 cursor-only 同步唤醒开关，但尚不发送可见的“新商机/AI 回复/每日摘要”通知；provider/灰度未配置时 UI 明确不可用，不声称偏好已经投递 |
...
| 交互式 Agent | RN capability 门控 Agent Tab | `/agent/interactive/turns*`、`/agent/interactive/gateway/v1/chat/completions`、`/agent/interactive/actions/*` | 部分实现 | P5 beta 已实现单调版本化工具集：schema v1 本地只读搜索/详情/消息；v2 只增加本地未发送草稿、幂等 status outbox 入队和认证在线认领；v3 只增加 `send_reply`。v3 在 pi `beforeToolCall` 暂停并显示目标、渠道、完整可编辑正文、风险与有效期，用户本次手势后才以编辑值申请无正文 hash 审计和最多 120 秒的一次性 purpose token。执行端重新检查 owner/device/turn/nonce/exact args/version/status/archive/adapter/外发开关并复用 `ManualReplyDelivery`；并发/重放最多调用一次 provider，结果不确定禁止自动重试。approval token 不进入模型、SQLite、SecureStore、日志或 URL；成功卡片只在服务端确认后出现。客户端上报最高支持 v3，仍能按服务端选择运行 v1/v2。生产默认仍为 v1，beta/gateway/external-actions/额度/白名单及 `IM_SEND_ENABLED` 关闭；真实 provider、授权 IM 沙箱、双真机外发、记忆和跨设备会话尚未验收，因此能力仍标为部分实现 |
...
| 通知偏好/每日摘要 | Web/iOS/Android 设置 | `/settings/notifications` | 部分实现 | 偏好真实持久化（见上“用户级通知偏好”）；P3 push 只唤醒同步且无正文，可见提醒和每日摘要仍未实现，不能声称偏好已产生用户通知 |
