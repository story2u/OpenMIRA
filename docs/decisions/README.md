...
- [ADR-0006：移动端作为现有 API 的瘦客户端](0006-mobile-app-thin-client.md)（superseded by ADR-0012）
...
- [ADR-0012：移动端迁移到 React Native + Expo 单代码库](0012-react-native-expo-mobile-client.md)（accepted）
- [ADR-0013：端上 pi Agent 优先，服务端网关与 runner 兜底](0013-device-agent-with-hosted-fallback.md)（accepted）
- [ADR-0014：先用服务端变更日志同步，不立即切换事件溯源权威](0014-server-change-feed-before-event-sourcing.md)（accepted）
- [ADR-0015：信息胃口采用端侧事件日志与独立同步流](0015-signal-appetite-event-stream.md)（accepted）
